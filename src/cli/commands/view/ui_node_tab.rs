...
pub(crate) fn draw_node_tab<B>(f: &mut Frame<B>, app: &mut App, area: Rect)
where
    B: Backend,
{
    ...
    B: Backend,
{
        ...
        .split(area);
    {
        ...
        let overview_text: Line = if let Some(node_stats) = &app.state.node_stats {
            Line::from(vec![
...
fn draw_node_charts<B>(f: &mut Frame<B>, app: &mut App, area: Rect)
where
    B: Backend,
{
...
fn draw_jvm_heap_chart<B>(f: &mut Frame<B>, app: &mut App, area: Rect)
where
    B: Backend,
{
...
fn draw_jvm_non_heap_chart<B>(f: &mut Frame<B>, app: &mut App, area: Rect)
where
    B: Backend,
{
...
fn draw_process_cpu_chart<B>(f: &mut Frame<B>, app: &mut App, area: Rect)
where
    B: Backend,
{
...
fn draw_process_file_descriptor_gauge<B>(f: &mut Frame<B>, app: &mut App, area: Rect)
where
    B: Backend,
{
    let usage: u16;
